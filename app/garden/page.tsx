  ...
  const [hoverReturn, setHoverReturn] = useState(false)
   ...
   useEffect(() => {
    ...
    if (saved && JSON.parse(saved).length > 0) {
      // Real plants exist — use them
      setPlants(JSON.parse(saved))
    } else {
      // No real plants yet — show demo plants at different stages
      const demoPlants = [
        { id: 1, name: "Math Homework", stage: 1, completed: false, plantTypeId: "cherry-blossoms" },
        { id: 2, name: "Science Project", stage: 4, completed: false, plantTypeId: "cherry-blossoms" },
